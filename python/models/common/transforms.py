...
class DigitEmbedTransform(nn.Module):
    def __init__(self, digit_embed_size):
    ...
    def get_embedding_dim(self):
        return self.digit_embedding.embedding_dim

    def get_output_shape(self, node_shape):
        return node_shape[:-1] + (self.get_embedding_dim() * node_shape[-1],)

class TextEmbedTransform(nn.Module):
    def __init__(self, model="jinaai/jina-embeddings-v2-base-code"):
...
class Transforms(nn.Module):
    def __init__(self, transforms):
        ...
        self.transform_map = {
            "None": NoneTransform(),
            "DigitEmbed": DigitEmbedTransform(64),
            "TextEmbed": TextEmbedTransform(),
        }

        self.transforms = nn.ModuleDict(
            {name: self.transform_map[transform] for name, transform in transforms.items()}
        )
...
if __name__ == "__main__":
    ...
    text_embed_transform = TextEmbedTransform()
