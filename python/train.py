...
def main():
    ...
    e, latest = find_latest_file(str(GraMI_wgts_dir / f"{model_name}_")+"*.pt")
    ...
    writer = SummaryWriter(log_dir=Path("runs") / get_log_dir_name(model_config["model_name"]))
    ...
    for i in range(e, epochs):
        ...
        if i % 100 == 0:
            torch.save(model.state_dict(), GraMI_wgts_dir / f"{model_name}_{i}.pt")
